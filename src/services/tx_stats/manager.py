...
class TxMetricsManager:
    def __init__(self, *, provider: MetricsProvider[TXStatisticsMetrics]) -> None:
    ...
    async def ensure_current(self) -> MetricsState[TXStatisticsMetrics]:
    ...
    async def _start(self, *, force_refresh: bool) -> MetricsState[TXStatisticsMetrics]:
    ...
    def _on_task_done(self, task: asyncio.Task[None]) -> None:
        try:
            ...
            logger.warning("TX metrics refresh task was cancelled")
        except Exception:
            logger.exception("TX metrics refresh task failed unexpectedly")
        finally:
